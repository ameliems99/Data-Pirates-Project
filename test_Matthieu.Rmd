...
```{r}
untar("./doi_10.5061_dryad.hm2jf__v1/SNP-alignment-largeDataset+Spar.tgz")  #including this line will make the folder in the same location for everyone
...
```{r}
DNA_str_set = DNAStringSet(as.character(all_chromosomes_set[1, 1:10])) # we create a DNAstrset form the sequences we have in the dataframe
```
...
colnames(Dist_Mat) = colnames(all_chromosomes_set)# add rownames and colnames
rownames(Dist_Mat) = colnames(all_chromosomes_set)
